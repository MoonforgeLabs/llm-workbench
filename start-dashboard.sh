#!/usr/bin/env bash
# 启动管理看板 HTTP 服务器
# 用法: ./start-dashboard.sh [端口] [--daemon]
...
PID_FILE="/tmp/litellm-dashboard-${PORT}.pid"
LOG_FILE="/tmp/litellm-dashboard-${PORT}.log"
...
echo "🚀 启动 LiteLLM Workbench 管理看板"
echo "================================"
echo ""
...
if [[ "$DAEMON_MODE" == "--daemon" ]]; then
  ...
  if is_running; then
    ...
    echo ""
    echo "📋 管理命令:"
    echo "   停止: kill $(cat "$PID_FILE") 或 rm $PID_FILE"
